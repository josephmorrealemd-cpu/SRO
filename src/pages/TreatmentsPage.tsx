...
import { Sparkles, GitFork, BookOpen, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="pt-12 min-h-screen">
      ...
      <div className="container mx-auto px-4 text-center mb-10">
        ...
        {/* Guided Funnel Bar */}
        <div className="mt-8 max-w-3xl mx-auto p-4 rounded-2xl bg-teal-50/80 border border-teal-200/80 flex flex-col sm:flex-row items-center justify-between gap-4 text-left shadow-xs">
          <div>
            <div className="text-xs font-bold uppercase tracking-wider text-teal-700 flex items-center gap-1.5 mb-1">
              <Sparkles className="w-3.5 h-3.5" /> Personalized Biologics Matching
            </div>
            <p className="text-xs text-slate-700 font-medium">
              Not sure whether Wharton’s Jelly, Exosomes, or PRP is right for your joint?
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <Link
              to="/pain-quiz"
              className="bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-all shadow-xs flex items-center gap-1"
            >
              Take 2-Min Quiz <ArrowRight className="w-3 h-3" />
            </Link>
            <Link
              to="/biologics-decision"
              className="bg-white hover:bg-slate-100 text-slate-800 border border-slate-200 text-xs font-bold px-3.5 py-2.5 rounded-xl transition-all"
            >
              Decision Tool
            </Link>
          </div>
